...
def length():
    with open("shops_moscow_coords.txt", "r", encoding='utf-8') as lines:
        return sum(1 for _ in lines)


def get_shops(location: str, k: int) -> list[str]:
    if (k == 0) or (abs(k) >= length()):
        with open("shops_moscow_coords.txt", "r", encoding='utf-8') as p_file:
            return [line.strip().split(' -- ')[0] for line in p_file.readlines()]
...
def create_cli():
    ...
    parser.add_argument("k", nargs='?', default=0, type=int, help="The number of closest shops to search.")
    parser.add_argument("location", nargs='?', default="г. Москва, улица Новый Арбат", type=str, help="Your approximate location.")
